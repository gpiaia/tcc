...
Existem muitas combinações de controladores PID, cada uma com suas peculiaridades e vantagens de uso. Uma combinação muito usada é a PI, onde a ação integral de zerar o erro em regime e uma boa resposta transitória já satisfazem as especificações. 
...
Com isso, podemos representar de forma discreta o controlador PID, que pode ser visto na equação da sequência:
...
Onde $U(S)$ é o sinal do controlador, $Y(S)$ é o sinal amostrado da saída da planta, $\hat {Y}(S)$ é a saída estimada, H é o ganho do filtro e A, B e C são as matrizes em espaço de estados da dinâmica da planta.
...
\begin{figure}[H]
  ...
  \begin{center}
      \includegraphics[scale=0.55]{referencial/img/robust_regression}
  \end{center}
